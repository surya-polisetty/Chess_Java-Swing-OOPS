...
public class Main extends JFrame{
	...
	private JPanel controlPanel, WhitePlayer, BlackPlayer, banner, displayTime, showCurrentPlayer, time;
	...
	public static void main(String[] args) {
		...
		wr01 = new Rook("WR01", "../pieces/White_Rook.png", 0);
		wr02 = new Rook("WR02", "../pieces/White_Rook.png", 0);
		br01 = new Rook("BR01", "../pieces/Black_Rook.png", 1);
		br02 = new Rook("BR02", "../pieces/Black_Rook.png", 1);
		wk01 = new Knight("WK01", "../pieces/White_Knight.png", 0);
		wk02 = new Knight("WK02", "../pieces/White_Knight.png", 0);
		bk01 = new Knight("BK01", "../pieces/Black_Knight.png", 1);
		bk02 = new Knight("BK02", "../pieces/Black_Knight.png", 1);
		wb01 = new Bishop("WB01", "../pieces/White_Bishop.png", 0);
		wb02 = new Bishop("WB02", "../pieces/White_Bishop.png", 0);
		bb01 = new Bishop("BB01", "../pieces/Black_Bishop.png", 1);
		bb02 = new Bishop("BB02", "../pieces/Black_Bishop.png", 1);
		wq = new Queen("WQ", "../pieces/White_Queen.png", 0);
		bq = new Queen("BQ", "../pieces/Black_Queen.png", 1);
		wk = new King("WK", "../pieces/White_King.png", 0, 7, 3);
		bk = new King("BK", "../pieces/Black_King.png", 1, 0, 3);
		wp = new Pawn[8];
		bp = new Pawn[8];
		for (int i = 0; i < 8; i++) {
			wp[i] = new Pawn("WP0" + (i + 1), "../pieces/White_Pawn.png", 0);
			bp[i] = new Pawn("BP0" + (i + 1), "../pieces/Black_Pawn.png", 1);
		}
	...
	private Main() {
		...
		timeSlider.addChangeListener(new TimeSetHandler());
		
		// Fetching Details of all previouis Players
		...
		controlPanel.setMaximumSize(new Dimension(285, 700));
		...
		WNewPlayer.addActionListener(new NewPlayerHandler(0));
		BNewPlayer.addActionListener(new NewPlayerHandler(1));
		...
		//timer
		showCurrentPlayer = new JPanel(new FlowLayout());
		showCurrentPlayer.add(timeSlider);
		...
		time.add(setTime);
		time.add(showCurrentPlayer);
		displayTime.add(start);
		time.add(displayTime);
		controlPanel.add(time);


		// BANNER: The Left Layout When Game is not started
		banner = new JPanel() {
			private static final long serialVersionUID = 1L;
		...
		banner.setMinimumSize(new Dimension(800, 700));
		...
		for (int j = 0; j < 8; j++) {
			...
			cell.addMouseListener(new MouseCellHandler());
			board.add(cell);
			boardState[i][j] = cell;
		}

		
		split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, banner, controlPanel);
	...
	// A function to change the turn from White Player to Black Player or vice
	// verse
	...
	public void changechance() {
		...
		if (!end && timer != null) {
			timer.reset();
			timer.start();
			showCurrentPlayer.remove(CHNC);
			if (Main.move == "White")
				Main.move = "Black";
			else
				Main.move = "White";
			CHNC.setText(Main.move);
			showCurrentPlayer.add(CHNC);
		}
	}
	...
	// is under Check
	private ArrayList<Cell> incheck_poss_moves_filter(ArrayList<Cell> destlist, Cell fromcell, int color) {
		ArrayList<Cell> newlist = new ArrayList<Cell>();
		Cell newboardstate[][] = new Cell[8][8];
		ListIterator<Cell> it = destlist.listIterator();
		int x, y;
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (boardState[i][j].getpiece() != null && boardState[i][j].getpiece().getcolor() == color) {
					...
					dlist = incheck_poss_moves_filter(dlist, boardState[i][j], color);
					if (dlist.size() != 0)
						return false;
				}
			}
	...
	private void gameend() {
		...
		displayTime.add(start);
		showCurrentPlayer.remove(mov);
		showCurrentPlayer.remove(CHNC);
		showCurrentPlayer.revalidate();
		showCurrentPlayer.add(timeSlider);
	...
	class MouseCellHandler implements MouseListener{
				...
				if (c.x == previous.x && c.y == previous.y) {
					...
					if (c.ispossibledestination()) {
						...
						if (getKing(turn ^ 1).isindanger(boardState)) {
							...
							if (isCheckmate(getKing(turn ^ 1).getcolor())) {
								previous.deselect();
	...
	class TimeSetHandler implements ChangeListener {
		@Override
